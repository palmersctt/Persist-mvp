...
function stablePick(tier: MoodTier, focus: number, strain: number, balance: number): Mood {
  ...
  const hash =
    dayOfYear * 31 + Math.round(focus) * 7 + Math.round(strain) * 13 + Math.round(balance) * 17;
...
/**
 * When the readiness algorithm has spoken (wearable connected), the card's
 * mood comes from ITS band — never a second, calendar-only opinion. One
 * verdict, one mood, same tier.
 */
export function moodFromReadinessBand(
  band: 'prime' | 'maintain' | 'recover',
  focus: number,
  strain: number,
  balance: number
): Mood {
  const tier: MoodTier = band === 'prime' ? 'good' : band === 'maintain' ? 'ok' : 'bad';
  return stablePick(tier, focus, strain, balance);
}
