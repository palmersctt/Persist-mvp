...
import { detectMood, moodFromReadinessBand, MOODS, MOOD_TIERS, type Mood } from './mood';
...
describe('moodFromReadinessBand – one algorithm, one mood', () => {
  it('maps prime to a good-tier mood', () => {
    expect(MOOD_TIERS.good).toContain(moodFromReadinessBand('prime', 80, 30, 75));
  });

  it('maps maintain to an ok-tier mood', () => {
    expect(MOOD_TIERS.ok).toContain(moodFromReadinessBand('maintain', 50, 50, 50));
  });

  it('maps recover to a bad-tier mood', () => {
    expect(MOOD_TIERS.bad).toContain(moodFromReadinessBand('recover', 80, 30, 75));
  });
});
