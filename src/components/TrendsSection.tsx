...
import { buildTrendsFromHistory, MIN_WEEKLY_DAYS, MIN_MONTHLY_WEEKS, type DailyScore } from '../lib/trends'
...
export default function TrendsSection({ history }: { history: DailyScore[] }) {
  ...
  if (!trends.weekly) {
    return (
      <section className="max-w-xs mx-auto w-full">
        <h2 className="text-[11px] font-bold uppercase mb-3" style={{ letterSpacing: '0.12em', color: 'var(--text-faint)' }}>
          Your Trends
        </h2>
        <div className="rounded-xl p-5" style={{ backgroundColor: 'var(--surface)', border: '1px solid var(--rule)' }}>
          <div className="flex items-center justify-between mb-3">
            <span className="text-sm font-bold" style={{ color: 'var(--text)' }}>Building your baseline</span>
          ...
          <div className="flex gap-1.5 mb-4">
            {Array.from({ length: MIN_WEEKLY_DAYS }).map((_, i) => (
              <div
                ...
                style={{ backgroundColor: i < trends.daysTracked ? 'var(--signal)' : 'var(--rule)' }}
              />
            ))}
          </div>
          <p className="text-xs leading-relaxed" style={{ color: 'var(--text-muted)' }}>
            Persist is learning what your normal looks like. Trends unlock after {MIN_WEEKLY_DAYS} tracked
            days &mdash; then every day compounds into the pattern.
          </p>
        </div>
      </section>
    )
  ...
  return (
    <section className="max-w-xs mx-auto w-full">
      <div className="flex items-center justify-between mb-3">
        <h2 className="text-[11px] font-bold uppercase" style={{ letterSpacing: '0.12em', color: 'var(--text-faint)' }}>
          Your Trends
        </h2>
        <span
      ...
      <div className="flex rounded-lg overflow-hidden mb-4" style={{ border: '1px solid var(--rule)' }}>
          ...
          return (
            <button
              ...
              style={{
                ...
                color: activeView === v ? 'var(--ground)' : disabled ? 'var(--text-faint)' : 'var(--text-muted)',
      ...
      <div className="flex flex-col gap-2.5">
        {metrics.map(({ key, label, getValue }) => {
          ...
          return (
            <div key={key} className="rounded-xl px-4 pt-3.5 pb-3" style={{ backgroundColor: 'var(--surface)', border: '1px solid var(--rule)' }}>
              <div className="flex items-center justify-between mb-2">
                <div className="flex items-center gap-2">
                  <div className="w-2 h-2 rounded-sm" style={{ backgroundColor: 'var(--signal)' }} />
                  <span className="text-[13px] font-bold" style={{ color: 'var(--text)' }}>{label}</span>
                </div>
                <div className="flex items-center gap-1">
                  <span className="text-lg font-bold" style={{ color: 'var(--signal)', fontFamily: 'var(--font-geist-mono), ui-monospace, monospace', fontVariantNumeric: 'tabular-nums' }}>
                    {current}
                  </span>
                  {(isUp || isDown) && (
              ...
              <svg width="100%" viewBox={`0 0 ${W} ${H}`} preserveAspectRatio="none" style={{ display: 'block', color: 'var(--signal)' }} aria-hidden="true">
                {area && <path d={area} fill="currentColor" opacity={0.08} />}
                {line && <path d={line} fill="none" stroke="currentColor" strokeWidth={2} strokeLinecap="round" strokeLinejoin="round" />}
                {pts.length > 0 && todayIdx >= 0 && todayIdx < pts.length && (
                  <>
                    <circle cx={pts[todayIdx].x} cy={pts[todayIdx].y} r={5} fill="currentColor" opacity={0.15} />
              ...
              {topInsight && (
                <p className="text-[11px] mt-2 mb-0 leading-snug" style={{ color: 'var(--text-muted)' }}>
                  {topInsight.title}
                </p>
              )}
            </div>
      ...
      {activeView === 'weekly' && (
        <div className="grid grid-cols-2 gap-2 mt-3">
          <div className="rounded-lg px-3.5 py-3" style={{ backgroundColor: 'var(--signal-soft)', border: '1px solid var(--signal-edge)' }}>
            <span className="text-[10px] font-bold uppercase" style={{ color: 'var(--signal)', letterSpacing: '0.1em' }}>Best day</span>
            <p className="text-[15px] font-bold mt-0.5 mb-0" style={{ color: 'var(--text)' }}>{trends.weekly.bestDay}</p>
          </div>
          <div className="rounded-lg px-3.5 py-3" style={{ backgroundColor: 'var(--signal-soft)', border: '1px solid var(--signal-edge)' }}>
            <span className="text-[10px] font-bold uppercase" style={{ color: 'var(--signal)', letterSpacing: '0.1em' }}>Hardest day</span>
            <p className="text-[15px] font-bold mt-0.5 mb-0" style={{ color: 'var(--text)' }}>{trends.weekly.worstDay}</p>
          </div>
        </div>
      )}
      ...
      {activeView === 'monthly' && trends.monthly && (
        <div className="text-center mt-3">
          <span className="text-xs font-bold" style={{ color: trends.monthly.trend === 'stable' ? 'var(--text-faint)' : 'var(--signal)' }}>
      ...
      {allInsights.length > 0 && (
        <details className="mt-3" onToggle={(e) => {
          ...
          <summary className="text-xs font-semibold cursor-pointer py-2 flex items-center gap-1.5 list-none" style={{ color: 'var(--text-faint)' }}>
            <span className="text-[10px]">&#9654;</span>
            All insights ({allInsights.length})
          </summary>
          <div className="pt-1">
            {allInsights.map((insight, i, arr) => (
              <div key={i} className="py-3" style={{ borderBottom: i < arr.length - 1 ? '1px solid var(--rule)' : 'none' }}>
                <div className="flex items-center gap-1.5 mb-1">
                  <div className="w-1.5 h-1.5 rounded-full flex-shrink-0" style={{ backgroundColor: 'var(--signal)' }} />
                  <span className="text-[13px] font-bold" style={{ color: 'var(--text)' }}>{insight.title}</span>
                </div>
                <p className="text-xs leading-relaxed pl-3 mb-0" style={{ color: 'var(--text-muted)' }}>{insight.message}</p>
              </div>
            ))}
          </div>
        </details>
