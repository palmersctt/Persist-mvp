...
const ZONE_CONFIG: Record<ZoneKey, {
  ...
  displacement: {
    label: 'Displacement',
    color: '#C7F95C',
    bg: 'rgba(199, 249, 92, 0.04)',
    tagBg: 'rgba(199, 249, 92, 0.08)',
    tagBorder: 'rgba(199, 249, 92, 0.2)',
...
const RISK_COLORS: Record<RiskLevel, { bar: string; bg: string; label: string }> = {
  total: { bar: '#A8DE3F', bg: 'rgba(199,249,92,0.08)', label: '#C7F95C' },
  'very-high': { bar: '#C7F95C', bg: 'rgba(199,249,92,0.06)', label: '#C7F95C' },
  high: { bar: '#C7F95C', bg: 'rgba(199,249,92,0.06)', label: '#C7F95C' },
  medium: { bar: '#5F6168', bg: 'rgba(245,245,245,0.08)', label: '#9B9DA3' },
  low: { bar: '#C7F95C', bg: 'rgba(199,249,92,0.06)', label: '#C7F95C' },
  'very-low': { bar: '#A8DE3F', bg: 'rgba(168,222,63,0.06)', label: '#A8DE3F' },
  none: { bar: '#5F6168', bg: 'rgba(245,245,245,0.04)', label: '#9B9DA3' },
}
...
function SignalBlock({ label, value, color, subtitle, isNeg, building }: {
  ...
  if (building) {
    return (
      <div style={{ flex: 1, padding: '20px 0' }}>
        <div style={{ fontSize: 10, fontWeight: 700, letterSpacing: '0.14em', textTransform: 'uppercase' as const, color: '#9B9DA3', marginBottom: 10 }}>
          {label}
        </div>
        <div style={{ fontSize: 16, fontWeight: 600, color: '#5F6168', lineHeight: 1.3, marginBottom: 8 }}>
          Building
        </div>
        <div style={{ height: 3, borderRadius: 2, background: '#23252B', overflow: 'hidden', marginBottom: 8 }}>
          <div style={{
            height: '100%', borderRadius: 2,
            background: 'repeating-linear-gradient(90deg, #23252B 0px, #23252B 8px, transparent 8px, transparent 16px)',
            width: '100%',
          }} />
        </div>
        <div style={{ fontSize: 12, color: '#5F6168', lineHeight: 1.3 }}>{building}</div>
      </div>
    )
  }
  ...
  return (
    <div style={{ flex: 1, padding: '20px 0' }}>
      ...
      <div style={{ fontSize: 12, color: '#5F6168', lineHeight: 1.3 }}>{subtitle}</div>
    </div>
  )
}
...
function TrendSpark({ data, color, height = 56 }: {
  ...
  return (
    <svg viewBox={`0 0 200 ${height + 16}`} style={{ width: '100%', height: height + 16, display: 'block' }} preserveAspectRatio="none">
      {[0, 1, 2].map(i => (
        <line key={i} x1="0" y1={i * (height / 2)} x2="200" y2={i * (height / 2)} stroke="#23252B" strokeWidth="0.5" />
      ))}
      <path
      ...
      {values.map((v, i) => {
        ...
        return (
          <g key={i}>
            ...
            <circle cx={x} cy={y} r={i === values.length - 1 ? 4 : 2.5} fill={i === values.length - 1 ? color : '#15161A'} stroke={color} strokeWidth="1.5" />
          </g>
        )
      })}
      {data.map((d, i) => (
        <text key={i} x={(i / (data.length - 1)) * 200} y={height + 14} textAnchor="middle" fontSize="9" fill="#5F6168" fontFamily="inherit">
          W{i + 1}
        </text>
      ))}
    </svg>
...
function BreakdownBar({ item, maxHours, events, isOpen, onToggle, highlight }: {
  ...
  const highlightBg = highlight === 'human' ? 'rgba(199,249,92,0.10)' :
                      highlight === 'risk' ? 'rgba(199,249,92,0.10)' : 'transparent'
  ...
  return (
    <div style={{
      ...
      >
        <div style={{ width: 140, fontSize: 13, fontWeight: 600, color: '#9B9DA3', flexShrink: 0 }}>{item.category}</div>
        ...
        <div style={{ width: 42, fontSize: 13, fontWeight: 700, color: '#9B9DA3', textAlign: 'right' as const, flexShrink: 0, fontVariantNumeric: 'tabular-nums' }}>{item.hours}h</div>
        <div style={{
        ...
        <div style={{ padding: '4px 8px 10px 20px' }}>
          {categoryEvents.map((evt, i) => (
            <div key={i} style={{
              ...
              padding: '5px 0',
              borderTop: i > 0 ? '1px solid rgba(245,245,245,0.5)' : 'none',
            }}>
              <span style={{ fontSize: 12, color: '#9B9DA3' }}>{evt.title}</span>
              <span style={{ fontSize: 11, color: '#5F6168', fontVariantNumeric: 'tabular-nums', flexShrink: 0, marginLeft: 12 }}>
...
function Skeleton() {
  const shimmer = {
    background: 'linear-gradient(90deg, #23252B 25%, #1F2024 50%, #23252B 75%)',
...
export default function CognitivePositioning() {
    ...
    return (
      <div style={{ minHeight: '100vh', background: '#0B0B0C', display: 'flex', alignItems: 'center', justifyContent: 'center' }}>
        <div style={{ textAlign: 'center' as const }}>
          <p style={{ fontSize: 16, color: '#9B9DA3', marginBottom: 16 }}>Sign in to see your AI Pulse.</p>
          <button
            onClick={() => signIn('google', { callbackUrl: '/positioning' })}
            style={{ padding: '12px 24px', borderRadius: 10, border: 'none', background: '#C7F95C', color: '#fff', fontWeight: 700, fontSize: 14, cursor: 'pointer' }}
          >
            Sign in with Google
          </button>
        </div>
  ...
  if (isLoading || status === 'loading') {
    return (
      <div style={{ minHeight: '100vh', background: '#0B0B0C', fontFamily: "'DM Sans', -apple-system, system-ui, sans-serif", color: '#F5F5F5' }}>
        <Skeleton />
      </div>
    )
  }
  ...
  if (error) {
    return (
      <div style={{ minHeight: '100vh', background: '#0B0B0C', display: 'flex', alignItems: 'center', justifyContent: 'center', fontFamily: "'DM Sans', -apple-system, system-ui, sans-serif" }}>
        <div style={{ textAlign: 'center' as const, maxWidth: 400 }}>
          <p style={{ fontSize: 16, color: '#9B9DA3', marginBottom: 8 }}>Something went wrong</p>
          <p style={{ fontSize: 13, color: '#5F6168', marginBottom: 16 }}>{error}</p>
          <button
            onClick={refresh}
            style={{ padding: '10px 20px', borderRadius: 8, border: '1.5px solid #23252B', background: '#15161A', color: '#F5F5F5', fontWeight: 600, fontSize: 13, cursor: 'pointer' }}
          >
            Retry
          </button>
        </div>
    ...
    return (
      <div style={{ minHeight: '100vh', background: '#0B0B0C', display: 'flex', alignItems: 'center', justifyContent: 'center', fontFamily: "'DM Sans', -apple-system, system-ui, sans-serif" }}>
        <div style={{ textAlign: 'center' as const, maxWidth: 420, padding: '0 24px' }}>
          <p style={{ fontSize: 18, fontWeight: 700, color: '#F5F5F5', marginBottom: 8 }}>No calendar events yet</p>
          <p style={{ fontSize: 14, color: '#9B9DA3', lineHeight: 1.5 }}>
  ...
  return (
    <div style={{
      ...
      <div style={{ maxWidth: 800, margin: '0 auto', padding: '40px 24px 60px' }}>
        ...
        <header className="sticky top-0 z-40 bg-[#15161A]/80 border-b border-[#23252B] backdrop-blur-sm" style={{ margin: '0 -24px', padding: '24px 24px 16px' }}>
          <div className="flex justify-between items-center">
            <Link href="/" className="flex items-center gap-2" style={{ textDecoration: 'none' }}>
              <PersistLogo size={24} variant="dark" />
              <span className="text-lg font-semibold" style={{ color: 'var(--text-primary, #F5F5F5)', letterSpacing: '1.5px' }}>PERSIST<span style={{ color: '#C7F95C' }}>WORK</span></span>
            </Link>
            <div className="flex items-center space-x-4">
              <button
                ...
                style={{
                  color: isLoading ? 'var(--text-muted, #5F6168)' : 'var(--text-secondary, #9B9DA3)',
                  border: `1px solid ${isLoading ? 'rgba(245,245,245,0.04)' : '#23252B'}`,
                  backgroundColor: 'transparent',
                  cursor: isLoading ? 'not-allowed' : 'pointer',
                }}
                onMouseEnter={(e) => { if (!isLoading) { e.currentTarget.style.backgroundColor = 'rgba(245,245,245,0.06)' } }}
              ...
              <div ref={profileRef} className="relative">
                ...
                >
                  {session?.user?.image ? (
                    ...
                    <div className="w-full h-full flex items-center justify-center text-xs font-bold" style={{ backgroundColor: 'rgba(245,245,245,0.06)', color: 'var(--text-secondary, #9B9DA3)' }}>
                    ...
                    <div className="p-4 border-b" style={{ borderColor: '#23252B' }}>
                      <div className="flex items-center gap-3">
                        ...
                        <div className="min-w-0">
                          <p className="text-sm font-medium truncate" style={{ color: 'var(--text-primary, #F5F5F5)' }}>{session?.user?.name}</p>
                          <p className="text-xs truncate" style={{ color: 'var(--text-muted, #5F6168)' }}>{session?.user?.email}</p>
                        </div>
                      </div>
                    </div>
                    <div className="p-4 border-b" style={{ borderColor: '#23252B' }}>
                      <p className="text-[10px] uppercase tracking-wider font-semibold mb-2" style={{ color: 'var(--text-muted, #5F6168)' }}>Permissions</p>
                      <div className="space-y-1.5">
                        <div className="flex items-center gap-2">
                          <span className="text-[#C7F95C] text-xs">&#10003;</span>
                          <span className="text-xs" style={{ color: 'var(--text-secondary, #9B9DA3)' }}>Email address</span>
                        </div>
                        <div className="flex items-center gap-2">
                          <span className="text-[#C7F95C] text-xs">&#10003;</span>
                          <span className="text-xs" style={{ color: 'var(--text-secondary, #9B9DA3)' }}>Profile info (name, photo)</span>
                        </div>
                        <div className="flex items-center gap-2">
                          <span className="text-[#C7F95C] text-xs">&#10003;</span>
                          <span className="text-xs" style={{ color: 'var(--text-secondary, #9B9DA3)' }}>Google Calendar (read-only)</span>
                        </div>
                      </div>
                    </div>
                    <div className="p-3">
                      <button
                        ...
                        style={{ color: 'var(--text-secondary, #9B9DA3)', backgroundColor: 'rgba(245,245,245,0.04)' }}
        ...
        <div style={{
          ...
          <div style={{ flex: 1, minWidth: 260 }}>
            ...
            <p style={{ fontSize: 14, color: '#9B9DA3', margin: 0, lineHeight: 1.5, maxWidth: 500 }}>
              {data.insight}
            </p>
          </div>
          <div style={{ display: 'flex', gap: 16, flexShrink: 0 }}>
            <div style={{ textAlign: 'center' as const, cursor: 'pointer' }} onClick={() => scrollAndHighlight('human')}>
              <div style={{ fontSize: 11, color: '#5F6168', marginBottom: 2 }}>Human</div>
              <div style={{ fontSize: 20, fontWeight: 800, color: '#C7F95C' }}>{data.humanHours}h</div>
            </div>
            <div style={{ width: 1, background: '#23252B' }} />
            <div style={{ textAlign: 'center' as const, cursor: 'pointer' }} onClick={() => scrollAndHighlight('risk')}>
              <div style={{ fontSize: 11, color: '#5F6168', marginBottom: 2 }}>At risk</div>
              <div style={{ fontSize: 20, fontWeight: 800, color: '#C7F95C' }}>{data.autoHours}h</div>
            </div>
          </div>
        </div>
        ...
        {/* Three Signals */}
        <div style={{
          background: '#15161A', border: '1px solid #23252B',
          borderRadius: 14, padding: '4px 28px', marginBottom: 24,
          display: 'flex', gap: 0,
        }}>
          <SignalBlock label="Leverage" value={signals.leverage} color="#C7F95C" subtitle="% time on human-essential work" />
          <div style={{ width: 1, background: '#23252B', margin: '16px 24px' }} />
          <SignalBlock label="Exposure" value={signals.exposure} color="#C7F95C" subtitle="% time in automation-risk work" />
          <div style={{ width: 1, background: '#23252B', margin: '16px 24px' }} />
          {signals.momentumReady ? (
            <SignalBlock
              label="Momentum"
              value={signals.momentum}
              color={signals.momentum >= 0 ? '#C7F95C' : '#C7F95C'}
        ...
        <div style={{ display: 'grid', gridTemplateColumns: hasTrend ? '1fr 280px' : '1fr', gap: 24 }}>
          ...
          <div ref={classificationRef} style={{
            ...
            <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: 14 }}>
              <div style={{ fontSize: 10, fontWeight: 700, letterSpacing: '0.12em', textTransform: 'uppercase' as const, color: '#9B9DA3' }}>
                Work Classification
              </div>
              <div style={{ fontSize: 12, color: '#5F6168', fontVariantNumeric: 'tabular-nums' }}>
            ...
            {/* Stacked summary bar */}
            <div style={{ marginTop: 16, paddingTop: 16, borderTop: '1px solid #23252B' }}>
              ...
              <div style={{ display: 'flex', justifyContent: 'space-between', marginTop: 6 }}>
                <span style={{ fontSize: 11, color: '#C7F95C', fontWeight: 600 }}>← Human-essential</span>
                <span style={{ fontSize: 11, color: '#C7F95C', fontWeight: 600 }}>Automatable →</span>
              </div>
            </div>
          </div>
          ...
          {hasTrend && (
            <div style={{ display: 'flex', flexDirection: 'column' as const, gap: 24 }}>
              ...
              <div style={{
                ...
                <div style={{ fontSize: 10, fontWeight: 700, letterSpacing: '0.12em', textTransform: 'uppercase' as const, color: '#9B9DA3', marginBottom: 4 }}>
                  Leverage — {data.weekData.length} Weeks
                </div>
                <div style={{ fontSize: 12, color: '#5F6168', marginBottom: 14 }}>
              ...
              <div style={{
                ...
                <div style={{ fontSize: 10, fontWeight: 700, letterSpacing: '0.12em', textTransform: 'uppercase' as const, color: '#9B9DA3', marginBottom: 14 }}>
                  Your Zone
                </div>
