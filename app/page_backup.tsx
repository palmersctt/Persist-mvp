...
class BiometricEngine {
  constructor() {
    // Simulate realistic Apple Watch data with day-to-day variation
    this.generateRealisticBiometrics();
    this.personalBaseline = this.establishBaseline();
    this.correlationData = this.loadCorrelationData();
  }

  // Generate realistic biometric data that varies based on meeting density
  generateRealisticBiometrics() {
    const today = new Date();
    const dayOffset = today.getDate() % 7; // Simulate week pattern
    
    // Simulate meeting impact on biometrics
    const meetingCount = this.getTodaysMeetings().length;
    const meetingImpact = Math.max(0, (meetingCount - 4) * -3); // Each meeting over 4 reduces metrics by 3%
    
    // Base values with daily variation
    const baseHRV = 45 + (dayOffset * 3) + meetingImpact; // 45-65ms range
    const baseSleep = 7.5 + (Math.sin(dayOffset) * 0.9) - (meetingCount > 6 ? 0.8 : 0); // 6.5-8.5h range
    const baseRHR = 52 + (dayOffset % 4) + Math.max(0, meetingCount - 5); // 52-58 bpm range
    
    this.rawBiometrics = {
      hrv: Math.round(baseHRV),
      sleepDuration: Math.round(baseSleep * 10) / 10,
      sleepQuality: Math.round((85 + (dayOffset * 2) - meetingImpact) * 10) / 10,
      restingHeartRate: Math.round(baseRHR),
      activityStrain: Math.round((8 + (meetingCount * 0.7)) * 10) / 10,
      timestamp: today
    };
  }

  // Calculate readiness score using research-based weights
  calculateReadinessScore() {
    const { hrv, sleepDuration, sleepQuality, restingHeartRate } = this.rawBiometrics;
    const baseline = this.personalBaseline;
    
    // Calculate component scores (0-100)
    const hrvScore = Math.min(100, Math.max(0, (hrv / baseline.hrv) * 100));
    const sleepScore = Math.min(100, Math.max(0, 
      (sleepDuration / 8.0) * 50 + (sleepQuality / 100) * 50
    ));
    const rhrScore = Math.min(100, Math.max(0, 
      100 - ((restingHeartRate - baseline.restingHeartRate) * 2)
    ));
    
    // Research-based weights: HRV 40%, Sleep 35%, RHR 25%
    const readinessScore = Math.round(
      (hrvScore * 0.40) + (sleepScore * 0.35) + (rhrScore * 0.25)
    );
    ...
    return {
      ...
      components: {
        ...
        heartRate: Math.round(rhrScore)
      }
    };
  }

  // Establish personal baseline from first week of data
  establishBaseline() {
    // Simulate established baseline (would be calculated from historical data)
    return {
      hrv: 52, // Personal average HRV
      restingHeartRate: 54, // Personal resting heart rate
      sleepDuration: 7.8, // Personal sleep need
      sleepQuality: 88 // Personal sleep quality average
    };
  }

  // Load correlation data between meetings and biometrics
  loadCorrelationData() {
    // Simulate learned correlations (would be calculated from historical data)
    return {
      meetingDensityImpact: {
        '0-3': { hrvChange: +2, sleepChange: +1, rhrChange: -1 },
        '4-5': { hrvChange: -3, sleepChange: -5, rhrChange: +2 },
        '6+': { hrvChange: -8, sleepChange: -12, rhrChange: +4 }
      },
      backToBackImpact: {
        effectiveness: -20,
        stressIncrease: +15,
  ...
  getCurrentBiometrics() {
    ...
    return {
      ...
      recovery: readiness.components.hrv + readiness.components.sleep > 150 ? 94 : 
                readiness.components.hrv + readiness.components.sleep > 120 ? 82 : 
                readiness.components.hrv + readiness.components.sleep > 100 ? 74 : 65,
      strain: this.rawBiometrics.activityStrain,
      hrv: this.rawBiometrics.hrv,
      sleep: `${this.rawBiometrics.sleepDuration}h`,
      status: readiness.score >= 85 ? 'READY' : 
              readiness.score >= 70 ? 'MODERATE' : 'RECOVERY'
    };
  }
  ...
  getTodaysRecommendations() {
    const biometrics = this.getCurrentBiometrics();
    const meetingCount = this.getTodaysMeetings().length;
    const readiness = this.calculateReadinessScore();
    ...
    const hrvDeviation = ((this.rawBiometrics.hrv - this.personalBaseline.hrv) / this.personalBaseline.hrv) * 100;
    if (hrvDeviation < -15) {
      recommendations.push({
        id: 1,
        type: 'recovery',
        message: `Your HRV is ${Math.abs(Math.round(hrvDeviation))}% below personal baseline (${this.rawBiometrics.hrv}ms vs ${this.personalBaseline.hrv}ms avg). Consider lighter cognitive load today.`
      });
    } else if (hrvDeviation > 15) {
      recommendations.push({
        id: 1,
        type: 'opportunity',
        message: `Excellent HRV today (+${Math.round(hrvDeviation)}% above baseline). Optimal window for high-stakes decisions and strategic work.`
      });
    }
    
    // Meeting density recommendations
    if (meetingCount >= 6) {
      const expectedImpact = this.correlationData.meetingDensityImpact['6+'];
      recommendations.push({
        id: 2,
        type: 'density_alert',
        message: `${meetingCount} meetings today. Your historical data shows 6+ meetings typically reduce tomorrow's HRV by ${Math.abs(expectedImpact.hrvChange)}% and sleep quality by ${Math.abs(expectedImpact.sleepChange)}%.`
      });
    }
    
    // Sleep-based recommendations
    if (this.rawBiometrics.sleepDuration < 7.0) {
      recommendations.push({
        id: 3,
        type: 'sleep_recovery',
        message: `Only ${this.rawBiometrics.sleepDuration}h sleep (${this.personalBaseline.sleepDuration}h personal target). Schedule buffer time between meetings for cognitive recovery.`
      });
    }
    
    // Default recommendation if others don't apply
    if (recommendations.length === 0) {
      recommendations.push({
        id: 1,
        type: 'baseline',
        message: `Readiness score ${readiness.score}% (HRV: ${this.rawBiometrics.hrv}ms, Sleep: ${this.rawBiometrics.sleepDuration}h). Good capacity for planned work today.`
      });
    }
      ...
      <div className="max-w-6xl mx-auto px-6 pb-8">
        ...
        {viewMode === 'dashboard' ? (
          ...
          <div className="space-y-8">
            ...
            /* Demo 2 Biometrics Dashboard - Complex Features */
            <div className="space-y-12">
              
              {/* Interactive Biometric Rings */}
              <section>
                <div className="flex flex-col sm:flex-row justify-center items-center gap-8 md:gap-12">
              ...
              <section>
                <h2 className="text-xl font-semibold text-white mb-6">Personal Biometric Patterns</h2>
                
                <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                  <div className="bg-gray-950 rounded-lg p-6 border border-gray-800">
                    <div className="flex justify-between items-center mb-3">
                      <h3 className="text-white font-medium">Meeting Density → HRV Impact</h3>
                      <span className="text-xs text-gray-500">Personal Data</span>
                    </div>
                    <div className="space-y-2 text-sm">
                      {Object.entries(engine.correlationData.meetingDensityImpact).map(([range, impact]) => (
                        <div key={range} className="flex justify-between">
                          <span className="text-gray-400">
                            {range === '0-3' ? '≤3 meetings:' : 
                             range === '4-5' ? '4-5 meetings:' : '6+ meetings:'}
                          </span>
                          <span className={
                            impact.hrvChange > 0 ? 'text-green-400' :
                            impact.hrvChange > -5 ? 'text-yellow-400' : 'text-red-400'
                          }>
                            {impact.hrvChange > 0 ? '+' : ''}{impact.hrvChange}% HRV avg
                          </span>
                        </div>
                      ))}
                    </div>
                    <div className="mt-3 pt-3 border-t border-gray-800">
                      <div className="text-xs text-gray-500">
                        Current: {engine.rawBiometrics.hrv}ms HRV ({engine.getTodaysMeetings().length} meetings today)
                      </div>
                    </div>
                  </div>
                  ...
                  <div className="bg-gray-950 rounded-lg p-6 border border-gray-800">
                    <div className="flex justify-between items-center mb-3">
                      <h3 className="text-white font-medium">Sleep vs Meeting Load</h3>
                      <span className="text-xs text-gray-500">7-day trend</span>
                    </div>
                    <div className="space-y-2 text-sm">
                      <div className="flex justify-between">
                        <span className="text-gray-400">Personal target:</span>
                        <span className="text-blue-400">{engine.personalBaseline.sleepDuration}h</span>
                      </div>
                      <div className="flex justify-between">
                        <span className="text-gray-400">Last night:</span>
                        <span className={engine.rawBiometrics.sleepDuration >= engine.personalBaseline.sleepDuration ? 'text-green-400' : 'text-orange-400'}>
                          {engine.rawBiometrics.sleepDuration}h
                        </span>
                      </div>
                      <div className="flex justify-between">
                        <span className="text-gray-400">Sleep quality:</span>
                        <span className="text-gray-300">{engine.rawBiometrics.sleepQuality}%</span>
                      </div>
                    </div>
                    <div className="mt-3 pt-3 border-t border-gray-800">
                      <div className="text-xs text-gray-500">
                        {engine.rawBiometrics.sleepDuration < engine.personalBaseline.sleepDuration ? 
                          `${(engine.personalBaseline.sleepDuration - engine.rawBiometrics.sleepDuration).toFixed(1)}h below target` :
                          'Meeting sleep target'
                        }
                      </div>
                    </div>
                  </div>
                ...
                <div className="mt-6 bg-gray-950 rounded-lg p-6 border border-gray-800">
                  <h3 className="text-white font-medium mb-3">Current Biometric Status</h3>
                  <div className="grid grid-cols-2 md:grid-cols-4 gap-4 text-sm">
                    <div>
                      <div className="text-gray-400">HRV</div>
                      <div className="text-white font-medium">{engine.rawBiometrics.hrv}ms</div>
                      <div className="text-xs text-gray-500">({engine.personalBaseline.hrv}ms avg)</div>
                    </div>
                    <div>
                      <div className="text-gray-400">Resting HR</div>
                      <div className="text-white font-medium">{engine.rawBiometrics.restingHeartRate}bpm</div>
                      <div className="text-xs text-gray-500">({engine.personalBaseline.restingHeartRate}bpm avg)</div>
                    </div>
                    <div>
                      <div className="text-gray-400">Sleep</div>
                      <div className="text-white font-medium">{engine.rawBiometrics.sleepDuration}h</div>
                      <div className="text-xs text-gray-500">{engine.rawBiometrics.sleepQuality}% quality</div>
                    </div>
                    <div>
                      <div className="text-gray-400">Strain</div>
                      <div className="text-white font-medium">{engine.rawBiometrics.activityStrain}</div>
                      <div className="text-xs text-gray-500">Daily load</div>
                    </div>
                  </div>
                </div>
              </section>
        ...
        ) : mainView === 'demo2' && demoTab === 'calendar' ? (
          /* Calendar View for Demo 2 */
          <div className="space-y-8">
